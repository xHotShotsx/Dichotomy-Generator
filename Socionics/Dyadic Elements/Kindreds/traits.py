...
def first_halves_balanced_splits(labels, base_label="Ne"):
...
def write_halves_to_file(path="Socionics/Dyadic Elements/Kindreds/kindred_splits_n8.txt", labels=TYPES, base_label="Ne"):
    with open(path, "w", encoding="utf-8") as f:
        for half in first_halves_balanced_splits(labels, base_label=base_label):
            f.write(",".join(half) + "\n")

def write_splits_to_file(path="Socionics/Dyadic Elements/Kindreds/kindred_splits_n8.txt", labels=TYPES, base_label="Ne"):
...
if __name__ == "__main__":
    ...
    print("Done. Generated 35 halves with Ne in the first half.")