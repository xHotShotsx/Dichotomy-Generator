...
def xnor16(a: int, b: int) -> int:
    return (~(a ^ b)) & FULL_MASK

def passes_symmetry(masks: list[int], i0: int, compare_js0: list[int]) -> bool:
    a = masks[i0]
    for j0 in compare_js0:
        if (a & masks[j0]).bit_count() != 4:
            return False
    return True
...
def report_unique_xnor(
    ...
    out_path: str = "Socionics/Types/xnor_unique_report.txt"
):
    ...
    for i in range(n, 0, -1):  # descending 6435 -> 1
        ...
        i0 = i - 1
        if not passes_symmetry(masks, i0, compare_js0):
            continue
...
if __name__ == "__main__":
    path = "Socionics/Types/socion_halves_n16.txt"
    compare = [2808, 2229, 426, 4082, 5266, 4407, 5083]  # your array
    kept_inputs, rows, out_file = report_unique_xnor(path, compare, out_path="Socionics/Types/xnor_unique_report.txt")
    print("Wrote:", out_file)
    print("Kept inputs:", len(kept_inputs))
    print("First 16 kept inputs:", kept_inputs[:16])
    print("First 16 mappings:", rows[:16])
